import CJS_COMPAT_NODE_URL_bvpgossdrrh from 'node:url';
import CJS_COMPAT_NODE_PATH_bvpgossdrrh from 'node:path';
import CJS_COMPAT_NODE_MODULE_bvpgossdrrh from "node:module";

var __filename = CJS_COMPAT_NODE_URL_bvpgossdrrh.fileURLToPath(import.meta.url);
var __dirname = CJS_COMPAT_NODE_PATH_bvpgossdrrh.dirname(__filename);
var require = CJS_COMPAT_NODE_MODULE_bvpgossdrrh.createRequire(import.meta.url);
...
import {
  any
} from "./_node-chunks/chunk-XPQHS7HE.js";
import {
  require_dist
} from "./_node-chunks/chunk-FMRZYTVN.js";
import {
  dirname,
  normalize,
  relative,
  resolve
} from "./_node-chunks/chunk-7JXKSHHN.js";
import {
  DOCUMENTATION_LINK
} from "./_node-chunks/chunk-MWIZ6PNK.js";
import {
  require_compare,
  require_constants,
  require_debug,
  ...
  require_re,
  require_semver
} from "./_node-chunks/chunk-HBC744T5.js";
import {
  __commonJS,
  __toESM
} from "./_node-chunks/chunk-LW4CMCYB.js";
...
var require_coerce = __commonJS({
  "../../../node_modules/semver/functions/coerce.js"(exports, module) {
    "use strict";
    var SemVer = require_semver(), parse = require_parse(), { safeRe: re, t: t2 } = require_re(), coerce = (version, options) => {
      if (version instanceof SemVer)
        return version;
...
var require_range = __commonJS({
  "../../../node_modules/semver/classes/range.js"(exports, module) {
    ...
    var LRU = require_lrucache(), cache = new LRU(), parseOptions = require_parse_options(), Comparator = require_comparator(), debug = require_debug(), SemVer = require_semver(), {
      ...
      let r = options.loose ? re[t2.CARETLOOSE] : re[t2.CARET], z = options.includePrerelease ? "-0" : "";
      return comp.replace(r, (_, M, m, p, pr) => {
    ...
    }, replaceStars = (comp, options) => (debug("replaceStars", comp, options), comp.trim().replace(re[t2.STAR], "")), replaceGTE0 = (comp, options) => (debug("replaceGTE0", comp, options), comp.trim().replace(re[options.includePrerelease ? t2.GTE0PRE : t2.GTE0], "")), hyphenReplace = (incPr) => ($0, from, fM, fm, fp, fpr, fb, to, tM, tm, tp, tpr) => (isX(fM) ? from = "" : isX(fm) ? from = `>=${fM}.0.0${incPr ? "-0" : ""}` : isX(fp) ? from = `>=${fM}.${fm}.0${incPr ? "-0" : ""}` : fpr ? from = `>=${from}` : from = `>=${from}${incPr ? "-0" : ""}`, isX(tM) ? to = "" : isX(tm) ? to = `<${+tM + 1}.0.0-0` : isX(tp) ? to = `<${tM}.${+tm + 1}.0-0` : tpr ? to = `<=${tM}.${tm}.${tp}-${tpr}` : incPr ? to = `<${tM}.${tm}.${+tp + 1}-0` : to = `<=${to}`, `${from} ${to}`.trim()), testSet = (set, version, options) => {
...
var require_comparator = __commonJS({
  "../../../node_modules/semver/classes/comparator.js"(exports, module) {
    ...
    var ANY = /* @__PURE__ */ Symbol("SemVer ANY"), Comparator = class _Comparator {
      ...
      parse(comp) {
        let r = this.options.loose ? re[t2.COMPARATORLOOSE] : re[t2.COMPARATOR], m = comp.match(r);
        if (!m)
...
import { babelParse as babelParse2, generate as generate2, traverse as traverse2 } from "storybook/internal/babel";
...
// src/angular-vitest-postinstall.ts
import {
  babelParse,
  generate,
  traverse,
...
var ANGULAR_VITEST_IMPORT_SOURCE = "@storybook/angular-vite/vitest", ANGULAR_VITEST_PLUGIN_CALL = "storybookAngularVitest", STORYBOOK_TEST_PLUGIN_SOURCE = "@storybook/addon-vitest/vitest-plugin", STORYBOOK_TEST_PLUGIN_CALL = "storybookTest";
function isAngularVitestAlreadyWired(content) {
  return content.includes(ANGULAR_VITEST_IMPORT_SOURCE) || content.includes(`${ANGULAR_VITEST_PLUGIN_CALL}(`);
}
function collectStorybookTestLocalNames(ast) {
  let names = /* @__PURE__ */ new Set([STORYBOOK_TEST_PLUGIN_CALL]);
  return traverse(ast, {
    ImportDeclaration(path) {
      path.node.source.value === STORYBOOK_TEST_PLUGIN_SOURCE && path.node.specifiers.forEach((specifier) => {
        "local" in specifier && specifier.local?.name && names.add(specifier.local.name);
      });
    }
  }), names;
}
function ensureImport(ast) {
  ...
  if (traverse(ast, {
    ImportDeclaration(path) {
      ...
      path.node.specifiers.some(
        (specifier) => specifier.type === "ImportSpecifier" && specifier.imported.type === "Identifier" && specifier.imported.name === ANGULAR_VITEST_PLUGIN_CALL
      ) && (hasImport = !0, path.stop());
    }
  }), hasImport)
    return;
  ...
  ), lastImportIndex = ast.program.body.findLastIndex((n) => n.type === "ImportDeclaration");
  ast.program.body.splice(lastImportIndex + 1, 0, importDecl);
}
function buildAngularVitestCall() {
  let call = t.callExpression(t.identifier(ANGULAR_VITEST_PLUGIN_CALL), [t.objectExpression([])]);
  return t.addComment(
    call,
    "leading",
...
function arrayHasAngularVitest(array) {
  return array.elements.some(
    (el) => el?.type === "CallExpression" && el.callee.type === "Identifier" && el.callee.name === ANGULAR_VITEST_PLUGIN_CALL
  );
}
function injectAngularVitestIntoAst(ast) {
  let localNames = collectStorybookTestLocalNames(ast), pluginsArray = null;
  if (traverse(ast, {
    CallExpression(path) {
      if (pluginsArray) {
        path.stop();
      ...
      callee.type === "Identifier" && localNames.has(callee.name) && path.parentPath.isArrayExpression() && (pluginsArray = path.parentPath.node, path.stop());
    }
  }), !pluginsArray)
    return !1;
  let array = pluginsArray;
  return arrayHasAngularVitest(array) || array.elements.unshift(buildAngularVitestCall()), ensureImport(ast), !0;
}
function injectAngularVitestIntoConfig(content) {
  let ast;
  try {
  ...
  return injectAngularVitestIntoAst(ast) ? generate(ast).code : null;
}
...
async function getTemplatePath(name) {
  switch (name) {
    case "vitest.config.template":
      return import("./_node-chunks/vitest.config.template-6A57UHWS.js");
    case "vitest.config.4.template":
      return import("./_node-chunks/vitest.config.4.template-S4KYIQ5K.js");
    case "vitest.config.3.2.template":
      return import("./_node-chunks/vitest.config.3.2.template-GSYDE4ML.js");
    case "vitest.workspace.template":
      return import("./_node-chunks/vitest.workspace.template-VNEVL6HR.js");
    default:
...
var loadTemplate = async (name, replacements) => {
  ...
  mergeProperties(properties, targetConfigObject.properties);
}, getWritableTargetConfigObject = (target, exportDefault) => {
  let directConfigObject = getTargetConfigObject(target, exportDefault);
  if (directConfigObject)
    return directConfigObject;
  let resolvedDecl = resolveExpression(exportDefault.declaration, target);
  if (resolvedDecl?.type !== "CallExpression" || !isDefineConfigLike(resolvedDecl, target))
    return null;
  let callbackArg = resolvedDecl.arguments[0];
  ...
  if (callbackArg.body.type === "BlockStatement" && callbackArg.body.body.length === 1 && callbackArg.body.body[0]?.type === "ReturnStatement") {
    let returnedExpr = resolveExpression(callbackArg.body.body[0].argument, target);
  ...
  if ((getWritableTargetConfigObject(target, targetExportDefault) !== null || getEffectiveMergeConfigCall(targetExportDefault.declaration, target) !== null) && (canHandleConfig = !0), !canHandleConfig)
    return !1;
  for (let sourceNode of source.program.body)
    if (sourceNode.type === "ImportDeclaration") {
      ...
      if (exportDefault && sourceNode.declaration.type === "CallExpression" && sourceNode.declaration.arguments.length > 0 && sourceNode.declaration.arguments[0].type === "ObjectExpression") {
        let { properties } = sourceNode.declaration.arguments[0], targetConfigObject = getWritableTargetConfigObject(target, exportDefault);
...
// src/postinstall.ts
var ADDON_NAME = "@storybook/addon-vitest", EXTENSIONS = [".ts", ".tsx", ".js", ".jsx", ".cts", ".mts", ".cjs", ".mjs"], STORYBOOK_TEST_PLUGIN_SOURCE2 = `${ADDON_NAME}/vitest-plugin`, addonA11yName = "@storybook/addon-a11y", getTemplateConfigDir = (configFilePath, configDir) => relative(dirname(configFilePath), configDir);
async function postInstall(options) {
  ...
  ), allDeps = packageManager.getAllDependencies(), addonVitestService = new AddonVitestService(packageManager), vitestVersionSpecifier = AddonVitestService.getComparableVersion(
...
`);
  ...
  let getTemplateName = (configContent) => isVitest4OrNewer ? "vitest.config.4.template" : isVitest3_2To4 ? configContent && configUsesWorkspace(configContent) ? "vitest.config.template" : "vitest.config.3.2.template" : "vitest.config.template", isAngularVite = info.framework === SupportedFramework.ANGULAR_VITE, maybeWireAngular = async (filePath, content, ErrorClass = AddonVitestPostinstallConfigUpdateError) => {
    if (!isAngularVite || isAngularVitestAlreadyWired(content))
      return;
    let injected = injectAngularVitestIntoConfig(content);
    if (injected === null) {
    ...
    let formattedContent = await formatFileContent(filePath, injected);
    await writeFile(filePath, formattedContent), logger.step("Added the @storybook/angular-vite standalone-vitest bridge.");
  };
  ...
  'ROOT_CONFIG',`, "").replace(/\s+extends: '',/, "")), source = babelParse2(workspaceTemplate), target = babelParse2(workspaceFileContent);
    if (updateWorkspaceFile(source, target)) {
      logger.step("Updating your Vitest workspace file..."), logger.log(`${vitestWorkspaceFile}`), isAngularVite && !isAngularVitestAlreadyWired(workspaceFileContent) && (injectAngularVitestIntoAst(target) ? logger.step("Added the @storybook/angular-vite standalone-vitest bridge.") : (logger.error(import_ts_dedent.dedent`
            We configured @storybook/addon-vitest, but could not automatically add the
      ...
      let formattedContent = await formatFileContent(vitestWorkspaceFile, generate2(target).code);
    ...
    if (templateName && !alreadyConfigured) {
      ...
      }), source = babelParse2(configTemplate);
      target = babelParse2(configFile), updated = updateConfigFile(source, target);
    }
    if (alreadyConfigured)
      await maybeWireAngular(rootConfig, configFile), logger.step(
        CLI_COLORS.success("Vitest for Storybook is already properly configured. Skipping setup.")
      );
    else if (target && updated) {
      logger.step(`Updating your ${vitestConfigFile ? "Vitest" : "Vite"} config file:`), logger.log(`  ${rootConfig}`), isAngularVite && !isAngularVitestAlreadyWired(configFile) && (injectAngularVitestIntoAst(target) ? logger.step("Added the @storybook/angular-vite standalone-vitest bridge.") : (logger.error(import_ts_dedent.dedent`
            We configured @storybook/addon-vitest, but could not automatically add the
            @storybook/angular-vite standalone-vitest bridge to:
            ${rootConfig}
      ...
      let formattedContent = await formatFileContent(rootConfig, generate2(target).code);
      await writeFile(
        rootConfig,
...
` + formattedContent : formattedContent
    ...
    if (isAngularVite) {
      let injected = injectAngularVitestIntoConfig(configTemplate);
      injected !== null && (configContent = injected, logger.step("Added the @storybook/angular-vite standalone-vitest bridge."));
    }
    let formattedContent = await formatFileContent(newConfigFile, configContent);
...
function isStorybookTestPluginSource(value) {
  return value === STORYBOOK_TEST_PLUGIN_SOURCE2;
}
function isConfigAlreadySetup(_configPath, configContent) {
  let ast;
  try {
    ast = babelParse2(configContent);
  } catch {
    return !1;
  }
